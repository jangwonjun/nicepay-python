...
def coupon_making():
    ...
    session['certification_code'] = certification_code
...
def registration_auto():
   ...
   certification_code = session.get('certification_code')
   
   try : 
        cursor.execute(f"USE {SQL.DB_NAME}")
        sql_query = "INSERT INTO pay (pay_num, pay_state) VALUES (%s, %s)"
        cursor.execute(sql_query, (certification_code,'0'))
        conn.commit()
        cursor.close()
        conn.close()
        print("쿠폰 정보가 입력되었습니다.")
...
def information():
    ...
    try : 
        #SQL 5칸
        cursor.execute(f"USE {SQL.DB_NAME}")
        sql_query = "INSERT INTO user (email, phone_number,coupon_number,coupon_state, pay_state,online_pay) VALUES (%s, %s, %s, %s, %s , %s)"
        cursor.execute(sql_query, (email,phone_number,int(certification_code),'0','0','0'))
        conn.commit()
        cursor.close()
        conn.close()
        print("사용자와 쿠폰 번호 정보가 입력되었습니다.")
    except pymysql.Error as e :
        print(f"SQL 구문 오류가 발생하였습니다. {e}")
